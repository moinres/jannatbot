...
@bot_admin
@run_async
def makesudo(bot: Bot, update: Update, args: List[str]):
...
@bot_admin
@run_async
def demotesudo(bot: Bot, update: Update, args: List[str]):
    message = update.effective_message
    user_id = extract_user(message, args)
...
__help__ = """
*@B0RNTOLE4RN:*
 - /makesudo: promotes the user to SUDO USER
 - /demotesudo: demotes the user from SUDO USER
"""

__mod_name__ = "🗣SUDO🗣"

makesudo_HANDLER = CommandHandler("makesudo", makesudo, pass_args=True, filters=Filters.user(OWNER_ID))
demotesudo_HANDLER = CommandHandler("demotesudo", demotesudo, pass_args=True, filters=Filters.user(OWNER_ID))

dispatcher.add_handler(makesudo_HANDLER)
dispatcher.add_handler(demotesudo_HANDLER)
